...
export class AnimationManager {
  constructor(vrm, audioMgr, renderer, scene, camera, controls) {
    ...
    this.config = {
      ...
      headAcceleration: 0.0015,     // Base acceleration (idle uses this for smooth look-around)
      ...
      stateConfig: {
        idle: {
          lookDuration: 2.0,           // Hold a look for 2 seconds (was 3.0 — more frequent changes)
          lookChangeChance: 0.45,      // Chance per second to change look (was 0.3)
          headEase: 0.014,             // Smooth easing
          headRangeX: 0.25,            // Max nod range
          headRangeY: 0.45,            // Max turn range to sides (was 0.75 — less extreme)
          headRangeZ: 0.18,            // Max tilt range
          eyeRange: 7.0,               // Eye look distance from center
          lookUpBias: 0.08,            // Slight upward bias so she's not staring down
          lookAtUserChance: 0.45,      // 45% chance to look back at user (was 0.35)
    ...
    if (this.idleLookingAtUser) {
      ...
      if (this.headTimer > cfg.lookDuration) {
        ...
        if (Math.random() < cfg.lookAtUserChance) {
          ...
          const upBias = cfg.lookUpBias || 0;

          this.headTgt.x = Math.sin(angle) * cfg.headRangeX * rangeMultiplier + upBias;
          ...
          this.eyeTgtPos.y = Math.cos(angle) * cfg.eyeRange * 0.4 + upBias * 8;
  ...
  update(deltaTime) {
    ...
    // VRM 0.x: rotateVRM0 rotates the scene 180° around Y, flipping scene local X and Z
    // axes relative to world space.  All proxy bone rotation.x and rotation.z values must
    // have their signs inverted for VRM 0.x; rotation.y is unaffected (Y axis unchanged).
    const isVRM0 = this.vrm.meta?.metaVersion === '0';
    const s = isVRM0 ? -1 : 1; // sign multiplier for X and Z axes

    // Apply head and neck rotations from state animations
    // This runs regardless of VRMA/Mixamo playing
    const neck = this.vrm.humanoid.getNormalizedBoneNode('neck');
    neck.rotation.set(s * this.headCur.x, this.headCur.y, s * this.headCur.z);
    ...
    // Use rotation.set() to own all axes — leaving x/y to the mixer while only
    // setting z causes the mixer's residual x/y to cascade into a twisted forearm.
    ...
    leftArm.rotation.set( 0, 0, isVRM0 ?  1.2 : -1.2 );
    rightArm.rotation.set( 0, 0, isVRM0 ? -1.2 :  1.2 );

    // Own the entire arm chain below the upper arm so no mixer residual
    // from Idle.fbx can leave child bones in positions inconsistent with
    // the forced upper arm orientation (which is what causes finger explosion).
    const armChain = [
    ...
    // Own the full torso chain so Idle.fbx mixer residuals can't tilt/lean the body
    ...
    spine.rotation.set(s * this.bodyCur.x, 0, 0);
